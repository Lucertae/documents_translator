...
def track_pdf_operation(operation: str):
    """
    Decorator to track PDF operations with Sentry.
    
    Adds breadcrumbs and captures exceptions automatically.
    
    Args:
        operation: Name of the operation (e.g., "load", "extract_text", "save").
    """
    def decorator(func: Callable) -> Callable:
        @wraps(func)
        def wrapper(*args, **kwargs):
            add_breadcrumb(
                message=f"Starting PDF operation: {operation}",
                category="pdf",
                level="info",
            )
            try:
                ...
                add_breadcrumb(
                    message=f"Completed PDF operation: {operation}",
                    category="pdf",
                    level="info",
                )
                return result
...
def track_translation(func: Callable) -> Callable:
    ...
    Captures translation errors with context about source/target languages.
    """
    @wraps(func)
    def wrapper(*args, **kwargs):
        add_breadcrumb(
        ...
        try:
            ...
            # Try to extract language info from args/kwargs
            context = {"function": func.__name__}
            if len(args) > 1:
                context["text_length"] = len(str(args[1])) if args[1] else 0
